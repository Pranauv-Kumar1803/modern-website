      ...
      <div className={`${styles.flexStart} md:flex-row flex-col w-full mb-8`}>
        ...
        <div className={`flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10`}>
          {footerLinks.map((footer,index)=>{
            return <div key={footer.title} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              <h4 className={`font-poppins font-medium text-[18px] leading-[27px] text-white`}> {footer.title} </h4>
              <ul className='list-none mt-4'>
                {footer.links.map((link,index)=>{
                  return <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index!=footer.links.length-1 ? 'mb-4' : 'mb-0'} `}> {link.name} </li>
                })}
              </ul>
            </div>
          })}
